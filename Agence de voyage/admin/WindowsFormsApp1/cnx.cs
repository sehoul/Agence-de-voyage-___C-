...
namespace WindowsFormsApp1
{
    class cnx
    {
        ...
        public static int search2(string table, string id)
        {
            DataSet _ds = new DataSet();
            int c = 0;
            da = new SqlDataAdapter("select count(*) from " + table + " where Matricule = '" + id + "'", cn);
            da.Fill(_ds, "trouve");
            c = int.Parse(_ds.Tables["trouve"].Rows[0][0].ToString());
        ...
        //n9edrou ndirou ghi bl shearch(); ?kifch ah# we ila nfss we tkhdm ok
